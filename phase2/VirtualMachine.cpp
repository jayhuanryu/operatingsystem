...
void VirtualMachine::ReadMemory(string infilename, int *progBase, int *progLimit) {
	...
	if(!readmemory.is_open()) {
		cout << infilename << " failed to open(1)" << endl;
		exit(1);
	}
...
void VirtualMachine::Execute(string filename){
	...
	clo = 0;
	while(clo <= 15) {// to be changed into infinite loop
...
void VirtualMachine::Read(int RD, ifstream& infile) {
...
void VirtualMachine::Write(int RD, string outfilename) {
